...
public
   ...
   enum MidiEventType
   {
      MidiEvent_Note = 0,        // Basic note event
      MidiEvent_SYSEX = 0xF0,    // System exclusive event
      MidiEvent_SYSEX2 = 0xF7,   // Another header for sysex
      MidiEvent_Meta = 0xFF      // Meta event
   };
   ...
   struct Header_Chunk
   {
      uint32	signature;     // "MThd", signature of a header chunk
      uint32	length;        // Length of the file (in bytes)
      uint16	format;        // Midi format
      uint16	numTracks;     // Number of tracks
      uint16	ticksPerBeat;  // Standard ticks/beat
   };

   // Midi track chunk, chunks out a track
   struct Track_Chunk
   {
      uint32	signature;  // "MTrk", signature of a track chunk
      uint32	length;     // Length of the track (in bytes)
   };

   // Standard structure for a generic midi event
   struct MidiEvent
   {
      uint32 tick;      // Tick number this event occurs on
      uint32 delta;     // Delta ticks between the last event
      uint32 type;      // The event type
      uint32 subType;   // The event subtype
